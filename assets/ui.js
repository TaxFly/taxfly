(function() {
  ...
  const BY_HREF = [ [ /tax\.html/, "calculator", "blue" ], [ /compras\.html/, "bag", "violet" ], [ /itinerario\.html/, "pin", "green" ], [ /unidades\.html/, "bulb", "amber" ], [ /tickets\.html/, "file", "cyan" ], [ /grupo\.html/, "users", "indigo" ], [ /rutas\.html/, "map", "red" ], [ /Maps\//, "ferris", "teal" ], [ /index\.html|^\.?\/?$/, "home", "blue" ] ];
  const norm = s => (s || "").replace(/[️‍]|\uD83C[\uDFFB-\uDFFF]/g, "").trim();
  function pick(el, text) {
    const a = el.matches(".ni, .dd-icon, .sc-emoji") ? el.closest("a[href]") : null;
    if (a) {
      const href = a.getAttribute("href") || "";
      for (const [re, icon, tone] of BY_HREF) if (re.test(href)) return {
        icon: icon,
        tone: tone
      };
    }
  ...
  const NAV_T = {
    es: {
      ...
      units: "Ayuda y referencias",
      units_d: "Conversor de unidades y ayudas varias",
      tickets: "Documentos",
    ...
    en: {
      ...
      units: "Help & references",
      units_d: "Unit converter & utilities",
      tickets: "Documents",
    ...
    pt: {
      ...
      units: "Ajuda e referências",
      units_d: "Conversor de unidades e utilidades",
      tickets: "Documentos",
      tickets_d: "ESTA, seguros, check-in",
      group: "Grupo",
  ...
  function swapText(root) {
    ...
    hits.forEach(([n, m, name, el]) => {
      ...
      const size = /^H[12]$/.test(el.tagName) ? 20 : fs >= 24 ? Math.round(fs) : fs >= 18 ? Math.round(fs * .95) : 16;
